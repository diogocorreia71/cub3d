...
run_valgrind() {
   ...
   valgrind ./cub3D "$map_file" &> "$log_file"
    local actual_status=$?
    
    if [ $actual_status -ne $expected_status ]; then
        echo -e "$map_file" # Red X
        echo "==== Falha no arquivo: $map_file ====" >> "$LOG_FILE"
        echo "Esperado: $expected_status, Obtido: $actual_status" >> "$LOG_FILE: WRONG"
        cat "$log_file" >> "$LOG_FILE"
        echo -e "\n\n" >> "$LOG_FILE"
        ((wrong_count++))
    else
        echo -e "$map_file: OK" # Green checkmark
        rm "$log_file"
        ((correct_count++))
    fi
}
