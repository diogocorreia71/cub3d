...
void	ft_ray_intersections(t_game *cub3d)
{
	if (cub3d->ray.direction.x < 0)
	{
		...
		cub3d->ray.intersection.x = (cub3d->player.pos.x - cub3d->player.int_pos_x) * cub3d->ray.delta.x;
	}
	else
	{
		cub3d->ray.map_mov.x = 1;
		cub3d->ray.intersection.x = (cub3d->player.int_pos_x + 1 - cub3d->player.pos.x) * cub3d->ray.delta.x;
	}
	if (cub3d->ray.direction.y < 0)
	{
		cub3d->ray.map_mov.y = -1;
		cub3d->ray.intersection.y = (cub3d->player.pos.y - cub3d->player.int_pos_y) * cub3d->ray.delta.y;
	}
	else
	{
		cub3d->ray.map_mov.y = 1;
		cub3d->ray.intersection.y = (cub3d->player.int_pos_y + 1 - cub3d->player.pos.y) * cub3d->ray.delta.y;
	}
}

void	ft_cast_a_ray(t_game *cub3d, int i)
{
	double	plane_var;//variacao no plano entre -1 e 1

	plane_var = 2 * i / (double)WIDTH - 1;
	cub3d->ray.direction.x = cub3d->player.direction.x + cub3d->player.plane.pos.x * plane_var; //vai mandar um raio na direcao do plane_var com o comprimento direction+plane
	cub3d->ray.direction.y = cub3d->player.direction.y + cub3d->player.plane.pos.y * plane_var; //vai mandar um raio na direcao do plane_var com o comprimento direction+plane
	cub3d->ray.delta.x = fabs(1 / cub3d->ray.direction.x);//calculo do delta = intersecoes na grid x
	cub3d->ray.delta.y = fabs(1 / cub3d->ray.direction.y);//calculo do delta = intersecoes na grid y
...
void    ft_distance_to_the_wall(t_game *cub3d)
{
	...
	cub3d->img_info.pos_texture = (cub3d->img_info.draw_start - (int)HEIGHT / 2 + cub3d->img_info.line_height / 2) * cub3d->img_info.scale;
